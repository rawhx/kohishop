...
public final class ListMenu {
    private static final Menu[] dataMakanan = {
    ...
    private static final Menu[] dataMinuman = {
    ...
    public static final void showMenu() {
        ...
        for (Menu mi : dataMinuman) {
            S.move(1, S.y); System.out.print(mi.getKode());
            S.move(10, S.y); System.out.print(mi.getNama());
            S.move(50, S.y++); System.out.print(mi.getHarga());
        }
    ...
    public static Menu cekMenuByKode(String kode) {
        for (Menu menu : dataMinuman) {
            if (menu.getKode().equalsIgnoreCase(kode)) return menu;
        }
        for (Menu menu : dataMakanan) {
            if (menu.getKode().equalsIgnoreCase(kode)) return menu;
        }
        return null;
    }
